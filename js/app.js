var Application = {
  ...
  initShowEvent: function () {
    $.ajax({
      url: 'https://projectppk-kel2.000webhostapp.com/select_data_tiket.php',
      type: 'get',
      beforeSend: function () {
        $.mobile.loading('show', {
      ...
      success: function (dataObject) {
        ...
        for (var i = 0; i < dataObject.length; i++) {
          ...
          dataObject[i].id_tiket +
            '"target="_self" id="detail-event" data-id_tiket="' +
            dataObject[i].id_tiket + '"><h2>' +
            dataObject[i].nama_tiket +
            '</h2><p><b>Rp. ' +
            dataObject[i].harga_tiket + '</b></p></a></li>';
